                        ...
                        <div class="group relative flex flex-col h-full bg-white border border-gray-100 rounded-lg hover:shadow-md transition-shadow">
                            ...
                            <div class="aspect-[4/5] w-full overflow-hidden rounded-t-lg bg-gray-200 relative">
                                ...
                                <div class="hidden lg:flex absolute bottom-4 left-0 right-0 justify-center opacity-0 group-hover:opacity-100 transition duration-300 gap-2">
                                     ...
                                     <form action="{{ route('wishlist.toggle', $product->id) }}" method="POST" class="relative z-20">
                                         @csrf
                                         <button type="submit" class="bg-white p-3 rounded-full shadow-lg hover:bg-red-50 transition transform hover:-translate-y-1">
                                            @php
                                                $isInWishlist = auth()->check() && auth()->user()->wishlists()->where('product_id', $product->id)->exists();
                                            @endphp
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="{{ $isInWishlist ? 'currentColor' : 'none' }}" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 {{ $isInWishlist ? 'text-red-500' : 'text-primary' }}">
